...
public class NewUserPacket {
    ...
    private final byte[] BYTE_ONLINE = new byte[] { 0x01 };
    ...
    public byte[] getLength() {
        ...
        int byteOnlineLen = BYTE_ONLINE.length;
    ...
    public byte[] getNewUserPacket() throws IOException {
        ByteArrayOutputStream newUserPacketComposit = new ByteArrayOutputStream();

        newUserPacketComposit.write(SERVER_COMMAND);
        newUserPacketComposit.write(getCount());
        newUserPacketComposit.write(getLength());
        newUserPacketComposit.write(getLengthWithNickname());
        newUserPacketComposit.write(BYTE_ONLINE);

        byte[] newUserPacket = newUserPacketComposit.toByteArray();
        ...
        System.out.println("BYTE_ONLINE " + java.util.HexFormat.ofDelimiter(" ").formatHex(BYTE_ONLINE));
